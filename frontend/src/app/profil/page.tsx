...
const FALLBACK_PROFIL: ProfilSekolahData = {
  ...
  visi: "Terwujudnya lulusan yang beriman, berakhlak mulia, berbudaya, cerdas, dan peduli lingkungan",
  misi: [
    "Menumbuhkan keimanan dan ketakwaan melalui pembiasaan ibadah, penguatan pendidikan agama, dan pengamalan nilai-nilai spiritual dalam kehidupan sehari-hari.",
    "Mengembangkan karakter berakhlak mulia dengan menanamkan nilai kejujuran, disiplin, tanggung jawab, kepedulian, serta sikap santun dalam interaksi sosial.",
    "Melestarikan dan mengembangkan budaya bangsa melalui pembelajaran, kegiatan ekstrakurikuler, serta apresiasi terhadap seni dan tradisi lokal maupun nasional.",
    "Meningkatkan kecerdasan peserta didik dengan pembelajaran yang aktif, kreatif, kolaboratif, inovatif, berbasis teknologi, serta mendukung potensi akademik dan non-akademik.",
    "Menumbuhkan kepedulian terhadap lingkungan hidup melalui kegiatan sekolah hijau, program kebersihan, penghijauan, pengelolaan sampah, dan pembiasaan gaya hidup ramah lingkungan.",
    "Membangun kemitraan dengan orang tua dan masyarakat untuk mendukung pendidikan yang holistik, seimbang, dan berkelanjutan.",
  ],
  strukturOrganisasi: [],
};
...
async function fetchStrukturFromRest(): Promise<StrukturOrganisasiItem[]> {
  try {
    ...
    if (Array.isArray(posts) && posts.length > 0) {
          ...
          Array.from(mediaIdsToFetch).map(async (mediaId) => {
            try {
              const mRes = await fetch(
                `${wpUrl}/wp-json/wp/v2/media/${mediaId}`,
              );
              if (mRes.ok) {
      ...
      const items: Array<StrukturOrganisasiItem & { order: number }> =
        posts.map((p: WpRestStruktur) => {
      ...
      return items.map(({ role, name, nip, category, fotoUrl }) => ({
        role,
        name,
        nip,
        category,
...
async function getProfilSekolah(): Promise<ProfilSekolahData> {
  ...
  try {
    const { data } = await fetchGraphQL<WPProfilResponse>(query, {
      revalidate: 60,
    });
    ...
    // 2. Atau jika diisi via teks textarea di Halaman Page "Profil"
      ...
      acf?.strukturOrganisasi &&
      acf.strukturOrganisasi.trim().length > 0
    ) {
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-[#111A4D] via-[#1E2B7A] to-[#0C1236] text-white py-16 lg:py-24">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-slate-200 text-base sm:text-lg max-w-3xl mx-auto mt-4 leading-relaxed">
            Menelusuri jejak sejarah, visi masa depan, komitmen mutu pendidikan,
            dan susunan pengelola yang berdedikasi tinggi demi kemajuan generasi
            bangsa.
          </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-center">
            <div className="lg:col-span-5 text-center lg:text-left">
              <div className="inline-flex items-center gap-2 text-[#1E2B7A] font-bold text-xs tracking-wider uppercase bg-blue-50 px-3.5 py-1.5 rounded-lg border-l-3 border-[#FFE500] mb-4">
                <History className="w-4 h-4 text-[#0097DF]" /> Kilas Balik &
                Rekam Jejak
              </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-300">
                      Berdiri sejak era awal kemerdekaan
                    </p>
                  </div>
                </div>
        ...
        <div className="space-y-10">
          ...
          <div className="text-center max-w-2xl mx-auto">
            ...
            <p className="text-slate-600 text-sm sm:text-base mt-2">
              Arah pandang dan langkah strategis dalam memandu seluruh proses
              kegiatan belajar mengajar di SMP Negeri 1 Ngawi.
            </p>
          </div>
                ...
                <div>
                  ...
                  <div className="p-5 space-y-2">
                    <div>
                      ...
                      <p className="text-xs text-[#0097DF] font-bold mt-1">
                        {item.name}
                      </p>
                    </div>
                    {item.nip && item.nip !== "-" && (
          ...
          <div className="bg-gradient-to-r from-[#111A4D] to-[#1E2B7A] rounded-3xl p-8 sm:p-10 text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl border border-[#0097DF]/30">
            <div className="space-y-2 text-center md:text-left">
              ...
              <p className="text-slate-300 text-xs sm:text-sm max-w-xl">
                Kunjungi direktori guru untuk melihat daftar lengkap pengajar
                per mata pelajaran, staf tata usaha, dan profil kualifikasi
                masing-masing.
              </p>
            </div>
            <Link
